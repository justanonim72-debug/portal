...
- At most two identities, assigned globally over both observations using palm proximity, bounded constant-velocity prediction and handedness confidence. Missing identities are retained for up to 500 ms, with visible identities taking priority when both slots are occupied. IDs are monotonically allocated and never reassigned to another finger by nearest-distance selection.
...
- A real CameraX preview/Recorder test records the front camera, records it again after background/resume before any camera switch, and then records the back camera. It decodes every MP4 and checks that the GPU violet filter is present. A separate unfiltered CameraX recording serves as a control. These tests supply deterministic panel geometry; they do not claim to validate real hand detection from emulator camera imagery.
